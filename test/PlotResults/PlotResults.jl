...
function RefErr_Lq(SC::Vector{Int64},nb::Int64,SCRef::Int64,nbRef::Int64;q::Real=2)
    ...
    function uref(x::Vector{Matrix{Float64}})
        ...
        solverref   = GetSolver(SCRef, nbRef)
    
        #Interpolate solution:
        
        u_terp,     = SolutionCompute(solverref.u, solverref.fes, [x1v, x2v])
        u_terpvm    = []
        
        for i=1:length(u_terp)
            
            push!(u_terpvm,reshape(u_terp[i],size(x[1])))
